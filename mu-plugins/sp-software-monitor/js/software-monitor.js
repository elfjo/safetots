jQuery(document).ready(function ($) {
    ...
    $("select#wp-core-updates-selector").change(function (event) {
        var $selector = $(this);

        $.post(SynthesisSoftwareMonitor.ajaxUrl,
            { 'action':'change_wp_core_updates', 'value': $selector.find(":selected").val() },
            function (data) {
                $("#sp-wp-core-update-message").html(data);
            }
        );
    ...
    $("input[name=https]").change(function (event) {
        var $checkbox = $(this);
        $.post(SynthesisSoftwareMonitor.ajaxUrl,
            { 'action':'change_sp_https_option', 'value': ( $checkbox.is(':checked') ) ? "on" : "off" },
            function (data) {
                $("#sp-has-ssl-message").html(data);
            }
        );
    ...
    $("button#archive-inactive-themes").click(function (event) {
        console.log("using themes section...");
        var $button = $(this);
        $button.attr('disabled', 'disabled');
        event.preventDefault();
        $.post(SynthesisSoftwareMonitor.ajaxUrl,
            { 'action':'take_theme_snapshot' },
            function (data) {
                var themeSnapshots = $('#theme-snapshots');
                themeSnapshots.html(data);
                collapseDetails('#theme-snapshots');
                themeSnapshots.find('div.collapsible').first().next().toggle('fast');
                $button.removeAttr('disabled');
            }
        );
    ...
    $('#theme-snapshots').on('click', '.delete-snapshot', function (event) {
        var $this = $(this);
        var snapshotId = $this.attr('data-snapshot-id');
        $.post(SynthesisSoftwareMonitor.ajaxUrl,
            {
                'action':'delete_theme_snapshot',
                'snapshot_id':snapshotId
            },
            function (data) {
                $this.parents('.smash-panel').remove();
