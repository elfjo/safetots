...
//		scribe_seo.custom_tb = true;
	...
	if($('#scribe-analysis').size() > 0) {
		...
		$('#scribe-content-analysis-analyze-button').click(function(event) {
			...
			if(undefined===headline) {
				if(typeof wp.data === 'object') {
					headline = wp.data.select("core/editor").getDocumentTitle()
				}
			}

			scribe_seo.analyze_content(title, description, content, post_id, headline, function(data,status) {
				if(!data.error) {
					scribe_seo.set_content_analysis_score(data.content_analysis.docScore, data.content_analysis.scribeScore);
...
var scribe_seo = function() {
	...
	object.set_content_analysis_score = function(page_score, site_score) {
		$('.scribe-analysis-meta-box-doc-score-container').find('.scribe-content-analysis-score').attr('data-content-analysis-score', page_score);
		$('.scribe-analysis-meta-box-doc-score-container').find('.scribe-content-analysis-score').text(page_score)

		$('.scribe-analysis-meta-box-site-score-container').find('.scribe-content-analysis-score').attr('data-content-analysis-score', site_score);
		$('.scribe-analysis-meta-box-site-score-container').find('.scribe-content-analysis-score').text(site_score)
	};
	...
	object.get_dependency_value = function(dependency_name) {
		...
		if (typeof wp.data === 'object' && 'content' == dependency_input_id) {
			var dependency_value = wp.data.select( "core/editor" ).getEditedPostContent().replace('/<(br\s?\/?|/?p|\!-- /?wp:paragraph --)>/','')
			if(dependency_value === '') {
				dependency_value = window._wpGutenbergPost.content
			}
		}
