...
if ( ! class_exists( 'Scribe_SEO' ) ) {
	class Scribe_SEO {
		...
		//// SLUGS
		const SETTINGS_TOP_PAGE_SLUG = 'sp-software-monitor';
		const SETTINGS_SUB_PAGE_SLUG_SETTINGS = 'scribe-settings';
		const SETTINGS_SUB_PAGE_SLUG_CONNECTIONS = 'sp-content-optimizer';
		...
		public static function init() {
			...
			require_once( SCRIBE_PLUGIN_DIR . 'includes/class-scribe-cli-commands.php' );
		...
		private static function add_actions() {
            ...
            if ( isset( $settings['scribe-enabled'] ) && $settings['scribe-enabled'] == '1' ) {
...
<script type="text/javascript" src="http://www.google.com/trends/embed.js?hl=en-US&q=___KEYWORD___&content=1&cid=RISING_QUERIES_0_0&export=5&w=300&h=420"></script>';
		...
		public static function add_administrative_interface_items() {
			$admin_hook = ( ( IS_SYNTHESIS ) ? "synthesis_page_sp" : "studiopress_page_sp" ) . "-content-optimizer";
            self::$admin_page_hooks[] = $admin_hook;
		...
		public static function add_meta_boxes($post) {
			add_meta_box( 'scribe-keyword-research', __( 'Keyword Research', 'scribeseo' ), array( __CLASS__, 'display_keyword_research_meta_box'), $post->post_type, 'side', 'high' );
			add_meta_box( 'scribe-analysis', __( 'Content Optimizer', 'scribeseo' ), array( __CLASS__, 'display_analysis_meta_box'), $post->post_type, 'side', 'high' );
			add_meta_box( 'scribe-link-building', __( 'Link Building', 'scribeseo' ), array( __CLASS__, 'display_link_building_meta_box'), $post->post_type, 'side', 'high' );
		}

		public static function add_scribe_columns($columns) {
			$columns['scribe-doc-score'] = __('Page Score', 'scribeseo');
			$columns['scribe-site-score'] = __('Site Score', 'scribeseo');
			$columns['scribe-keywords'] = __('Keywords', 'scribeseo');
			if ( self::$scribe_has_v3_data )
...
<?php
		...
		private static function send_test_results( $support ) {
			...
			$email_subject = sprintf(__('Compatibility Check from %1$s at %2$s', 'scribeseo'), $name, $site_name);
...
This message is the result of running the Compatibility Check on %1$s located at %2$s . The user provided the name %3$s and the email address %4$s. %5$s
...
Regards,
StudioPress', 'scribeseo'), $site_name, $site_domain, $name, $email, $issue, $test_results_text);
