...
if(!class_exists('Scribe_API')) {
	...
	class Scribe_API {
		...
		public function analyze_content( $title, $description, $content, $headline, $keyword, $url ) {
			...
			return new WP_Error( 'scribe_api_unknown_error', __( 'An unknown error occurred when contacting the API service.', 'scribeseo' ) );
		...
		private function send_request( $path, $get_data = array(), $post_data = array(), $is_post = false, $verify_key = true, $trailing_slash = true ) {
			...
			if(empty($this->_api_key) && $verify_key) {
				return new WP_Error('scribe_api_empty_api_key', __('An API Key must be provided in order to make requests from the API service.', 'scribeseo'));
			}
			...
			if ( ! is_wp_error( $response ) ) {
				...
				if ( null === $object )
					$result = new WP_Error( 'scribe_api_could_not_decode_response', __( 'The system could not decode the response from the API server.', 'scribeseo' ) );
				else
					$result = $object;
