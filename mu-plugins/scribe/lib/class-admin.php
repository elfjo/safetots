...
abstract class Scribe_Admin {
	...
	public function maybe_add_submenu() {
		...
		if ( isset( $this->menu_ops['submenu'] ) && is_array( $this->menu_ops['submenu'] ) ) {
			...
			$this->pagehook = add_submenu_page( $this->page_id, $menu['page_title'], $menu['menu_title'], $menu['capability'], "sp-content-optimizer", array( $this, 'admin' ) );
...
abstract class Scribe_Admin_Boxes extends Scribe_Admin {
	...
	public function admin() {
        ...
        if ( isset( $settings['scribe-enabled'] ) && $settings['scribe-enabled'] == '1' ) {
            Scribe_SEO::display_connections_page();
        }?>
