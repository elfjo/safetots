...
class Synthesis_Security {
    ...
    public static function add_menu_page() {
        $page = add_submenu_page( 'sp-software-monitor', __( 'Security Settings' ), __( 'Security' ), 'manage_options', 'security', array( __CLASS__, 'settings_page' ) );
        ...
        if ( isset( $_POST['synthesis-disable'] ) || isset( $_POST['synthesis-enable'] ) ) {
            ...
            if ( wp_verify_nonce( $nonce, self::NONCE_KEY ) && current_user_can( 'manage_options' ) ) {
                if ( $disable ) {
                    ...
                    self::$admin_message = sprintf( __( 'Security has been disabled for %d minutes' ), self::DISABLE_LENGTH );
                } else {
                    delete_transient( self::DISABLE_KEY );
                    self::$admin_message = sprintf( __( 'Security successfully enabled' ) );
                }
    ...
    ?>
        ...
        <?php screen_icon( 'synthesis-management' ); ?>
            <h2><?php _e( 'Security Settings' ); ?></h2>
            <p>
                <?php _e( 'Our native security blocks malicious requests to protect your site from attacks. If you need to, you can disable it temporarily below.' ); ?>
            </p>
            <form method="post" action="">
                <?php wp_nonce_field( self::NONCE_KEY, self::NONCE_KEY ); ?>
                <?php if ( get_transient( self::DISABLE_KEY ) ) { ?>
                <p>
                    <span class="description" style="color: red;"><?php _e( 'Security is currently disabled' ); ?></span>
                </p>
                <input type="submit" name="synthesis-enable" class="button-primary" value="<?php _e( 'Enable Security' ); ?>" />
                <?php } else { ?>
                <p>
                    <span class="description" style="color: green;"><?php _e( 'Security is currently enabled' ); ?></span>
                </p>
                <input type="submit" name="synthesis-disable" class="button-primary" value="<?php _e( 'Temporarily Disable Security' ); ?>" />
                <?php } ?>
            </form>
        </div>
    <?php
    ...
    public static function check_wp_firewall_2() {
        if ( is_plugin_active( 'wp-firewall-2/wp-firewall-2.php' ) ){
            self::show_message( 'WordPress Firewall 2 is active. Synthesis hosting deploys similar functionality via the Must-Use plugin Security. Disable and Delete WordPress Firewall 2 to avoid duplicate efforts and performance implications.' );
        }
    }
    ...
    public static function maybe_block_request() {
        ...
        if( ! self::check_whitelisted_ip() ) {
            ...
            $attack_vector_arrays = array(
                ...
                'sql-injection-attack' => array( 'concat' => 'concat\s*\(', 'group_concat' => 'group_concat', 'union' => 'union.*select.*(' . $wpdb->prefix . '|information_schema)', 'insertion' => '(\\\'|\%27).*(\-\-|\#|\%23)' ),
            ...
            foreach ( $filtered_request as $request_variable => $request_value ) {
                foreach( $attack_vectors as $attack_type => $regex ) {
                    if ( $attack_type == "field-truncation-attack" && ! preg_match( '/(user(name)?|login)/', $request_variable ) ) {
                        continue;
                    }
    ...
    public static function log_authentication_failure( $username ) {
        ...
        if ( ! class_exists( "Synthesis_Blacklist" ) ) {
            require_once( plugin_dir_path( __FILE__ ) . 'lib/synthesis-blacklist.php' );
        }
