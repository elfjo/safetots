...
class Scribe_Main_Settings extends Scribe_Admin_Boxes {
	...
	function __construct() {
		...
		$menu_ops = array(
			...
			'submenu' => array( /** Do not use without 'main_menu' */
				'page_title'	=> __( 'Content Optimizer Settings', 'scribeseo' ),
				'menu_title'	=> __( 'Content Optimizer', 'scribeseo' ),
				'capability'	=> 'manage_options'
			),
		);
...
#        $menu_ops = array();
	...
	function metaboxes() {
		...
		add_meta_box( 'scribe-main-settings', __( 'Content Optimizer Settings', 'scribeseo'), array( $this, 'main_settings_metabox' ), $this->pagehook, 'main' );
...
<table class="form-table">
	<tbody>
		<tr valign="top">
			<th scope="row">
				<label for="scribe-api-key"><?php esc_html_e( 'Enable Content Optimizer', 'scribeseo' ); ?></label>
				<a class="scribe-help-marker" rel="popover" title="<?php esc_html_e( 'Enable Content Optimizer', 'scribeseo' ); ?>" data-content="<?php esc_attr_e( 'When Content Optimizer is enabled, you will have a lot of tools to help you optimize your content.', 'scribeseo' ); ?>" href="#">?</a>
			</th>
			<td>
				<input type="checkbox" name="<?php echo $this->get_field_name( 'scribe-enabled' ); ?>" id="<?php echo $this->get_field_id( 'scribe-enabled' ); ?>" <?php checked( "1", $this->get_field_value( 'scribe-enabled') ); ?> value="1" />
			</td>
		</tr>
		<tr valign="top">
			<th scope="row">
...
<?php
			...
			if ( is_wp_error( $dependencies ) ) {
				...
				if ( 0 == $cumulative_count ) {
					printf( __( 'Please install and activate a valid SEO tool. You can see a full list of tools supported by the Content Optimizer on the <a href="%s">compatibility</a> page.', 'scribeseo' ), 'http://scribeseo.com/compatibility/' );
...
?>
		...
		<tr valign="top">
			<th scope="row">
				...
				<a class="scribe-help-marker" rel="popover" title="<?php _e( 'Permissions', 'scribeseo' ); ?>" data-content="<?php _e( 'The Content Optimizer enables you to control which user roles have access to the Content Optimizer. Click the drop down box to restrict Permissions to Administrators, Editors, Authors, or Contributors or higher. Set the lowest Role within WordPress that is available to use the Content Optimizer.', 'scribeseo' ); ?>" href="#">?</a>
			</th>
			<td>
		...
		<tr valign="top">
			<th scope="row">
				...
				<a class="scribe-help-marker" rel="popover" title="<?php esc_attr_e( 'Post Types', 'scribeseo' ); ?>" data-content="<?php esc_attr_e( 'By default, Posts and Pages in WordPress can use the Content Optimizer. If you have other Custom Post types, please select them for use. Add a check mark next to each type of post that you want to be able to run the Content Optimizer. No check mark indicates that the Content Optimizer will not be availble when editing the respective post type.', 'scribeseo' ); ?>" href="#">?</a>
			</th>
			<td>
				<ul>
	...
	function sanitize( $newvalue, $option ) {
		...
		elseif ( ! empty( $newvalue['seo-tool'] ) && empty( $newvalue['your-url'] ) )
			add_settings_error( '', 'scribe-url-invalid', __( 'The Content Optimizer requires Your URL to function properly.', 'scribeseo' ), 'error' );
